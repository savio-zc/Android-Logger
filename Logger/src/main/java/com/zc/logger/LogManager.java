...
//		checkConfig();
        ...
        log(null, text, new LogOption.Builder(tag, level).build(), null);
    }

    public void log(Throwable error, String text, LogOption local, LogManagerConfig global) {
        log(getMessage(error, text, local), local, global);
    }

    private LogMessage getMessage(Throwable error, String text, LogOption local) {
        if (local == null) {
            LogUtil.w(TAG, "local config: " + local);
            return null;
        }
        if (error == null && TextUtils.isEmpty(text)) {
            return null;
        }
        ...
        message.setThrowable(error);
        return message;
    }

    private void log(LogMessage message, LogOption local, LogManagerConfig global) {
